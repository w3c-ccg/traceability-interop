...
const urlExist = async (url) => {
  ...
  // using lt 500 to work around some issues on .well-known
    ...
    newman.run(
      {
        ...
        reporter: {
          json: {
            export: `${program.opts().reportdir}/service-provider-report.json`,
          },
          htmlextra: {
            export: `${program.opts().reportdir}/service-provider-report.html`,
          },
        },
      },
      (err) => {
