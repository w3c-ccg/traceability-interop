...
OAuth 2.0 `client_secret` values are stored as GitHub repository-level secrets. The naming convention used for these secrets is "CLIENT_SECRET_\<FQDN\>", where "\<FQDN\>" is the value of `serviceProvider.baseURL` from `client-configuration.json` with non-alpha-numerics converted to underscores.

For example, if the `baseURL` is "example.com", the `oauth2.client_secret_envvar` should be "CLIENT_SECRET_EXAMPLE_COM".
